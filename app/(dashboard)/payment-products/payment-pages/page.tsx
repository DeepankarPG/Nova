...
import { Ban, Copy, MoreHorizontal, Package, Pencil, Plus, Search, X } from "lucide-react";
...
import { paymentPagesSeed, type PaymentPageSummary } from "@/lib/mock-data/payment-page-create";

function currencySymbol(currency: string) {
  return currency === "USD" ? "$" : currency === "EUR" ? "€" : currency === "GBP" ? "£" : "₹";
}

const STATUS_TABS: { id: "all" | PaymentPageSummary["status"]; label: string }[] = [
  { id: "all", label: "All" },
  { id: "live", label: "Live" },
...
const STATUS_OPTIONS: { id: PaymentPageSummary["status"]; label: string }[] = [
  { id: "live", label: "Live" },
  { id: "paused", label: "Paused" },
  { id: "draft", label: "Draft" },
];
...
export default function PaymentPagesListPage() {
  const router = useRouter();
  const [pages] = useState<PaymentPageSummary[]>(paymentPagesSeed);
  const [activeTab, setActiveTab] = useState<"all" | PaymentPageSummary["status"]>("all");
  const [search, setSearch] = useState("");
  const [statusMulti, setStatusMulti] = useState<Set<PaymentPageSummary["status"]>>(new Set());
  const [amountRange, setAmountRange] = useState<AmountRange>({ min: "", max: "" });

  const goToDetail = (row: PaymentPageSummary) => router.push(`/payment-products/payment-pages/${row.id}`);
  ...
  const filteredPages = useMemo(() => {
    return pages.filter((row) => {
      ...
      if (search && !(row.product?.name || row.title).toLowerCase().includes(search.toLowerCase())) return false;
  ...
  const columns: Column<PaymentPageSummary>[] = [
    {
      ...
      render: (row) => (
        ...
        >
          ...
          <span className="truncate text-[13px] font-semibold text-foreground">{row.product?.name || row.title}</span>
        </button>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (row) => (
        <span className="group/link flex min-w-0 items-center gap-1.5">
          <span className="min-w-0 truncate text-muted-foreground">pay.payglocal.in/{row.slug}</span>
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();
              navigator.clipboard.writeText(`https://pay.payglocal.in/${row.slug}`).catch(() => {});
              toast.success("Link copied");
            }}
            aria-label="Copy link"
            className="shrink-0 text-muted-foreground opacity-0 transition-opacity hover:text-foreground group-hover/link:opacity-100"
          >
      ...
      <PageHeader
        ...
        actions={
          <Button
            ...
            onClick={() => router.push("/payment-products/payment-pages/create")}
          >
            Create payment page
          </Button>
        }
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        ...
        <div className="px-5 pb-5">
          <DataTable
            ...
            emptyTitle="No payment pages yet"
            emptyDescription="Create a branded payment page to start collecting payments."
          />
        </div>
      </div>
    </div>
