...
import { StandardChartTooltip, chartTooltipShellClass } from "@/components/charts/StandardChartTooltip";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import { ChartSkeleton } from "@/components/ui/skeleton";
import { todaysAnalytics } from "@/lib/mock-data";
...
function payModeDonutSegmentPath(
  cx: number,
  cy: number,
  rInner: number,
  rOuter: number,
...
function PaymentModeDonutSvg({
  segments,
  centerLabel,
  animateKey,
}: {
  segments: readonly { name: string; value: number; fill: string }[];
  centerLabel: string;
  /** Changes when tooltip point / mix changes — drives enter animation */
  animateKey: string;
}) {
  ...
  segments.forEach((seg) => {
    ...
    arcs.push({
      ...
      d,
      fill: seg.fill,
      fillRule: span >= 2 * Math.PI - 1e-5 ? "evenodd" : "nonzero",
    });
  });
...
function VolumeAnalyticsTooltip({
  active,
  payload,
  label,
  isPaymentsTab,
  ...
  /** Payment-mode donut + legend — gross volume only */
  showGrossPayMode: boolean;
}) {
  ...
  const row = payload[0]?.payload as {
    today?: number;
    yesterday?: number;
    payMixToday?: readonly [number, number, number, number];
    payMixYesterday?: readonly [number, number, number, number];
  };

  const formatMain = (v: number) =>
    isPaymentsTab
      ? new Intl.NumberFormat("en-IN").format(Math.max(0, Math.round(v)))
      : `₹${Number(v).toFixed(1)}L`;

  const mixT = row?.payMixToday ? normPayMix(row.payMixToday) : null;
  const todayTotal = Number(row?.today ?? 0);

  const payModeChartData = mixT
    ? VOLUME_PAY_MODE_LABELS.map((name, i) => ({
        name,
        value: Math.max(0, todayTotal * mixT[i]),
        fill: VOLUME_PAY_MODE_SWATCHES[i],
      }))
    : [];

  const showPayModeBlock =
    showGrossPayMode && mixT && payModeChartData.some((d) => d.value > 0);

  const payModeAnimateKey = showPayModeBlock
    ? `${String(label)}-${payModeChartData.map((d) => d.value.toFixed(4)).join("|")}`
    : "";
  ...
  return (
    <div
      className={cn(
        ...
        showPayModeBlock
          ? "max-w-[min(100vw-1.5rem,22.5rem)]"
          : "max-w-[min(100vw-1.5rem,17.5rem)]"
      )}
    >
        ...
        {payload.map((entry, i) => (
          <div key={i} className="flex items-center justify-between gap-3">
            ...
            <span className="shrink-0 text-[13px] font-semibold tabular-nums text-foreground">
              {entry.value !== undefined ? formatMain(Number(entry.value)) : "—"}
            </span>
          </div>
        ))}
      </div>
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] sm:items-stretch sm:gap-0">
            <div className="flex min-w-0 flex-col gap-2.5 sm:pr-4">
              ...
              <div className="flex justify-center rounded-xl border border-zinc-200/70 bg-zinc-50/90 px-3 py-3 dark:border-zinc-700/60 dark:bg-zinc-900/40 sm:justify-center">
                <PaymentModeDonutSvg
                  segments={payModeChartData}
                  centerLabel={formatMain(todayTotal)}
                  animateKey={payModeAnimateKey}
                />
              </div>
            </div>
                ...
                return (
                  <li key={name} className="flex items-center justify-between gap-3 text-[12px]">
                    <span className="flex min-w-0 items-center gap-2 text-muted-foreground">
                      <span
                        className="h-2.5 w-2.5 shrink-0 rounded-[3px] ring-1 ring-black/[0.06] dark:ring-white/[0.08]"
                        style={{ background: VOLUME_PAY_MODE_SWATCHES[i] }}
                        aria-hidden
                      />
                      <span className="truncate">{name}</span>
                    </span>
                    <motion.span
                      key={`${name}-${formatMain(vToday)}`}
...
function formatInrPlainNumber(n: number) {
...
/** Readable Y ticks for payment counts (0, 200, 400, …) */
...
function VolumeChartYTick({ y, payload, isPaymentsTab }: any) {
  ...
  const text = isPaymentsTab ? formatInrPlainNumber(Number(v)) : `₹${v}L`;
  return (
    <text
      x={0}
      y={y}
...
/** Flush left with headline: start anchor at gutter x=0 (margin.left is 0; width reserves space). */
...
export function TodaysAnalyticsSection({ isLoading }: { isLoading?: boolean }) {
  ...
  const chartRows = useMemo(() => {
    return todaysAnalytics.hourly.map((row) => ({
      label: row.label,
  ...
  const psrChartData = useMemo(
    () =>
      todaysAnalytics.hourly.map((r) => ({
        label: r.label,
  ...
  const psrYDomain = useMemo((): [number, number] => {
    ...
    const pad = 2;
    return [Math.max(0, Math.floor(lo - pad) - 1), Math.min(100, Math.ceil(hi + pad) + 1)];
  }, [psrChartData]);
  ...
  const yAxisConfig = useMemo(() => {
    if (isPaymentsTab) {
      const max = Math.max(
        ...chartRows.flatMap((r) => [Number(r.today), Number(r.yesterday)]),
        1
      );
  ...
  const headlineMain = useMemo(() => {
    if (tab === "payments") {
      return new Intl.NumberFormat("en-IN").format(summary.todayTotal);
    }
    return `₹${formatInrPlainNumber(summary.todayTotal as number)}`;
  }, [tab, summary]);
  ...
  return (
    <section className="grid gap-2 lg:grid-cols-12 lg:items-stretch lg:gap-3" aria-label="Today's analytics">
      <div
        className={cn(
          cardClass,
          "flex min-h-[300px] flex-col p-3 lg:col-span-8 lg:min-h-[316px]"
        )}
      >
        <div className="flex flex-col gap-3 pt-0.5 sm:flex-row sm:items-start sm:justify-between">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div
                className={cn(
                  "mt-1 flex flex-wrap items-center gap-1 text-[12px] font-semibold",
                  changePositive ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"
                )}
              >
                {changePositive ? (
                  <TrendingUp className="h-[13px] w-[13px] shrink-0" strokeWidth={2} aria-hidden />
                ) : (
                  <TrendingDown className="h-[13px] w-[13px] shrink-0" strokeWidth={2} aria-hidden />
                )}
          ...
          <LayoutGroup id="volume-metric-tabs">
            <div
              className="flex shrink-0 flex-wrap gap-0 rounded-lg border border-border bg-muted/45 p-1 dark:bg-muted/25"
              role="tablist"
            ...
            >
              {TABS.map((t) => (
                <button
                  ...
                  onClick={() => setTab(t.id)}
                  className="relative rounded-md px-2.5 py-1.5 text-left text-xs font-medium transition-colors sm:px-3 sm:text-[13px]"
                >
                  {tab === t.id && (
                    <motion.span
                      layoutId="volume-tab-pill"
            ...
            >
              <div className="min-h-[220px] w-full flex-1 px-1 pb-0.5 pt-1 sm:min-h-[236px] sm:px-2 sm:pb-1 sm:pt-1">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart
                    data={chartRows}
                    margin={{ top: 4, right: 10, left: 0, bottom: 12 }}
                  >
                    <defs>
                    ...
                    <YAxis
                      ...
                      allowDecimals={isPaymentsTab ? false : true}
                      axisLine={false}
                      tickLine={false}
                      width={isPaymentsTab ? 48 : 44}
                      tick={<VolumeChartYTick isPaymentsTab={isPaymentsTab} />}
                    />
                    <Tooltip
                      content={(props) => (
                        <VolumeAnalyticsTooltip
                          {...props}
                          isPaymentsTab={isPaymentsTab}
                          showGrossPayMode={tab === "gross"}
                        />
                      )}
                    />
                    <Area
                      ...
                      strokeWidth={2.25}
                      fill={`url(#ta-fill-${gradId})`}
                      dot={false}
                      activeDot={{ r: 5, strokeWidth: 0, fill: CHART_PRIMARY }}
                    />
                    <Line
                      type="monotone"
                      dataKey="yesterday"
                      name="Yesterday"
                      stroke={CHART_YESTERDAY}
                      strokeWidth={1.75}
                      strokeDasharray="5 4"
                      dot={false}
                      activeDot={{ r: 4, strokeWidth: 0, fill: CHART_YESTERDAY }}
                    />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
        ...
        <div
          className={cn(
            "mt-4 -mx-3 -mb-3 overflow-hidden rounded-b-xl border-t border-border/60",
      ...
      <div className="flex flex-col gap-2 lg:col-span-4">
        <div className={cn(cardClass, "flex flex-col p-3")}>
          ...
          <p className="mt-3 font-sans text-[1.75rem] font-bold leading-none tracking-[-0.02em] text-foreground tabular-nums">
            {successRate.pct}%
          </p>
          ...
          <div className="mt-2 h-[92px] w-full sm:h-[100px]">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={psrChartData} margin={{ top: 4, right: 8, left: 0, bottom: 6 }}>
                ...
                <Tooltip
                  content={(props) => (
                    <StandardChartTooltip {...props} formatValue={(v) => `${Number(v).toFixed(1)}%`} />
                  )}
                />
                <Area
                  type="monotone"
                  ...
                  stroke="#059669"
                  strokeWidth={2}
                  fill={`url(#psr-fill-${psrGradId})`}
                  dot={false}
                  activeDot={{ r: 3, strokeWidth: 0, fill: "#059669" }}
                />
                <Line
                  type="monotone"
                  dataKey="psrYesterday"
                  name="Yesterday"
                  stroke={CHART_YESTERDAY}
                  strokeWidth={1.5}
                  strokeDasharray="4 3"
          ...
          <div className="mt-2 border-t border-border pt-2.5">
            <div className="grid grid-cols-3 gap-1">
              <div className="text-left">
                ...
                <p className="mt-1 flex items-center justify-start gap-1.5 font-sans text-sm font-bold tabular-nums tracking-[-0.02em] text-foreground">
                  <span className="h-2 w-2 shrink-0 rounded-[3px] bg-emerald-500" />
                  {new Intl.NumberFormat("en-IN").format(successRate.succeeded)}
                </p>
              </div>
              <div className="text-left">
                <p className="text-[11px] font-medium text-muted-foreground">Failed</p>
                <p className="mt-1 flex items-center justify-start gap-1.5 font-sans text-sm font-bold tabular-nums tracking-[-0.02em] text-foreground">
                  <span className="h-2 w-2 shrink-0 rounded-[3px] bg-red-500" />
                  {new Intl.NumberFormat("en-IN").format(successRate.failed)}
                </p>
              </div>
              <div className="text-left">
                <p className="text-[11px] font-medium text-muted-foreground">Avg. value</p>
                <p className="mt-1 font-sans text-sm font-bold tabular-nums tracking-[-0.02em] text-foreground">
                  {formatInrCompact(successRate.avgTicket)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-3 space-y-1.5">
            <div className="flex items-center justify-between gap-2 rounded-lg border border-border bg-muted/30 p-2.5 dark:bg-muted/15">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[11px] font-normal leading-snug text-muted-foreground">
                  {attention.fundsOnHold.releaseLabel}
                </p>
              </div>
              <Button
                variant="outline"
                ...
                rightIcon={<ArrowUpRight className="h-3 w-3 shrink-0" strokeWidth={2} aria-hidden />}
              >
                Take action
              </Button>
            </div>
            <div className="flex items-center justify-between gap-2 rounded-lg border border-border bg-muted/30 p-2.5 dark:bg-muted/15">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 font-sans text-sm font-bold tabular-nums tracking-[-0.02em] text-red-800 dark:text-red-400 sm:text-base">
                  {formatInrCompact(attention.disputes.disputedAmount)}
                </p>
                <p className="mt-1 text-[11px] font-normal leading-snug text-muted-foreground">
                  {attention.disputes.openCount} open {attention.disputes.openCount === 1 ? "case" : "cases"}
                </p>
              </div>
              <Button
                variant="outline"
