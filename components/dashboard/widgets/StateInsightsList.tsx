...
export function StateInsightsList({
  ...
  if (isLoading) {
    return (
      <div
        className={cn("bg-card text-card-foreground rounded-xl p-5 border border-border shadow-sm", className)}
      >
    ...
    >
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Domestic INR volume by state (GST relevant)</p>
        </div>
          ...
          return (
            <div key={item.code} className="flex items-center gap-3">
              <div className="flex items-center gap-1.5 w-32 flex-shrink-0">
                <span className="text-[10px] font-semibold text-muted-foreground w-6">{item.code}</span>
                <span className="text-xs text-muted-foreground font-medium truncate">{item.state}</span>
              </div>
