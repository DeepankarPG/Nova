...
/** Above in-app overlays (e.g. DatePicker z-10000); below browser chrome in practice */
...
/** Max distance from (ox,oy) to any viewport corner — ensures full cover. */
...
export function ThemeTransitionProvider({ children }: { children: ReactNode }) {
  ...
  /* Portal to body so `position:fixed` is viewport-anchored (no transform traps) and paints above the app */
  useEffect(() => {
    setPortalReady(true);
  }, []);
    ...
    (btn: HTMLElement | null) => {
      ...
      const frame = (now: number) => {
        ...
        if (t < 1) {
          ...
          /* Solid fill matches post-toggle `body` background — no visible snap when canvas is removed */
